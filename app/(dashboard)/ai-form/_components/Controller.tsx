...
import { GetSocialAuthOption, ToggleSocialAuth } from '@/actions/form'

function Controller({ selectedTheme, selectedBackground, selectedStyle, id }: { selectedTheme: any, selectedBackground: any, selectedStyle: any, id: string }) {
  const [showMore, setShowMore] = useState(6);
  const [signInEnableValue, setSignInEnableValue] = useState<boolean>(false);

  useEffect(() => {
    (async () => {
      const result = await GetSocialAuthOption({ id: parseInt(id) });
      setSignInEnableValue(result?.enabledSignIn!);
    })();
  }, [])
  ...
  return (
    <div>
      ...
      <div className='flex gap-2 my-4 items-center mt-10'>
        <Checkbox checked={signInEnableValue} onCheckedChange={(e) => {
          setSignInEnableValue(e as boolean)
          ToggleSocialAuth({ id: parseInt(id), socialAuth: !signInEnableValue })
